...
def generate(aps):
    ...
    for ap in aps:
        ...
        if not ifname:
            # The bound radio isn't present right now. Skip rather than silently
            # retarget to wlan0 (which would hijack the onboard interface).
            print(f"hostapd.generate: AP id={ap.get('id')} ssid={ap.get('ssid')!r} "
                  f"bound to absent interface {ap['iface_uuid']}; skipping")
            continue
